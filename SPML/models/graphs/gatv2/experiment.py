...
class GATv2(Module):
    ...
    def __init__(self, in_features: int, n_hidden: int, out_features: int, n_heads: int, dropout: float, is_concat: bool = True,
        ...
        * `share_weights` if set to True, the same matrix will be applied to the source and the target node of every edge
        ...
        self.layer1 = GraphAttentionV2Layer(in_features, n_hidden, n_heads,
                                            is_concat, dropout=dropout, share_weights=share_weights)
        # Activation function after first graph attention layer
        self.activation = nn.ELU()
        # Final graph attention layer where we average the heads
        self.output = GraphAttentionV2Layer(n_hidden, out_features, 1,
                                            is_concat, dropout=dropout, share_weights=share_weights)
        # Dropout
    ...
    def forward(self, x: torch.Tensor, adj_mat: torch.Tensor):
        """
        * `x` is the features vectors of shape `[n_nodes, in_features]` label_emb
