...
class train_logger:
    ...
    def get_metric(self, phase, epoch, variant):
        '''
        Query the stop metric.
        '''
        assert variant in ['clean', 'observed']
        k_metric = self.params['metric'].keys()
        for k in k_metric:
            self.params['metric'][k] = self.logs['metrics'][phase][epoch][k + '_' + variant]

    def report(self, t_i, t_f, phase, epoch):
        report = '[{}] time: {:.2f} min, loss: {:.3f}'.format(
            phase,
            (t_f - t_i) / 60.0,
            self.logs['metrics'][phase][epoch]['loss'],
            )
        print(report)
        self.logg.info(report)
        self.get_metric(phase, epoch, 'clean')
        self.logg.info(self.params['metric'])
