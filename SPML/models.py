...
from models.misc import convert_models_to_fp32
...
def read_clip():
    clip_model = CLIPVisionModelWithProjection.from_pretrained(os.path.join(model_dir,'clip-vit-large-patch14-336'),output_attentions=True)
    return clip_model
...
class ImageClassifier(torch.nn.Module):
    ...
    def __init__(self, P, Z, model_feature_extractor=None, model_linear_classifier=None):
        ...
        if self.arch == 'clip_vision':
            ...
            clip_model = CLIPVIT(P, clip_model)
            convert_models_to_fp32(clip_model)
            self.feature_extractor = clip_model.to(Z['device'])
            self.vision_extractor = self.feature_extractor
            ...
            self.text_extractor = CLIPTextModelWithProjection.from_pretrained(os.path.join(model_dir,'clip-vit-large-patch14-336'),output_attentions=True)
            ...
            self.feature_extractor = CLIPVisionModelWithProjection.from_pretrained(os.path.join(model_dir,'clip-vit-large-patch14-336'),output_attentions=True)
    ...
    def forward(self, x, xs, y):
        '''
        X: batch['image'] [bs,3,448,448]
        xs: resized_batch [bs,3,224,224]
        原本:y: P['txt_features'] [classnum,768] 更改为label_embed [classnum,512]
        '''
        if self.arch == 'clip_vision':
            
            logits, feat= self.feature_extractor(xs, y)
            return logits, feat
...
class MultilabelModel_baseline(torch.nn.Module):
    def __init__(self, P, Z, feature_extractor, linear_classifier):
        ...
        self.f = ImageClassifier_baseline(P, Z, feature_extractor, linear_classifier)
